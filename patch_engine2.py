...
new_extract = '''    def extract_features(self, flows: List[NetworkObservation]) -> FeatureVector:
        if not flows:
            raise ValueError("Cannot extract features from empty flow list")
        ...
        dns_ent = sum(shannon(q) for q in dns_queries) / len(dns_queries) if dns_queries else 0.0
        tls_ent = sum(shannon(s) for s in tls_snis) / len(tls_snis) if tls_snis else 0.0
        
        # Entity Profiling (Host Baseline)
        host_ip = flows_sorted[0].source_ip
        if self.host_manager:
            profile = self.host_manager.get_profile(host_ip)
            h_conns = profile.get("connections_5m", 0)
            h_dests = profile.get("unique_dests_5m", 0)
            h_ports = profile.get("unique_ports_5m", 0)
            h_dns = profile.get("dns_queries_5m", 0)
            h_tls = profile.get("tls_connections_5m", 0)
            h_bytes_out = profile.get("bytes_out_5m", 0)
            h_bytes_in = profile.get("bytes_in_5m", 0)
        else:
            h_conns = h_dests = h_ports = h_dns = h_tls = h_bytes_out = h_bytes_in = 0
        ...
        return FeatureVector(
            ...
            host_connections_5m=h_conns,
            host_unique_dests_5m=h_dests,
            host_unique_ports_5m=h_ports,
            host_dns_queries_5m=h_dns,
            host_tls_connections_5m=h_tls,
            host_bytes_out_5m=h_bytes_out,
            host_bytes_in_5m=h_bytes_in
        )'''

content = re.sub(r'    def extract_features\(self, flows: List\[NetworkObservation\]\) -> FeatureVector:.*?(?=    def _calculate)', new_extract, content, flags=re.DOTALL)
