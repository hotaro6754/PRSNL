...
new_extract = '''    def extract_features(self, flows: List[NetworkObservation]) -> Optional[FeatureVector]:
        if not flows:
            return None
...
# find end of class
import sys
content = re.sub(r'    def extract_features\(self, flows: List\[NetworkObservation\]\) -> Optional\[FeatureVector\]:.*', new_extract, content, flags=re.DOTALL)
