...
def test_url(name, url, expected_status=200):
    ...
    try:
        ...
        # Evidence has 'feature', not 'evidence_type' in the API response
        pw_evidence = [e for e in data.get('evidence', []) if e.get('feature', '').startswith('Playwright')]
        if pw_evidence:
            for ev in pw_evidence:
                print(f"  [PLAYWRIGHT] Feature: {ev['feature']} | Value: {ev.get('value')} | Explanation: {ev.get('explanation')}")
        else:
